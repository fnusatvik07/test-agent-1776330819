...
CORS_ORIGINS = [
    ...
    f"http://localhost:8024",
    f"http://localhost:8025",
    f"http://localhost:8026",
    f"http://127.0.0.1:8024",
    f"http://127.0.0.1:8025",
    f"http://127.0.0.1:8026"
]
...
Core Principles:
- Provide precise, well-structured answers to all questions
- Maintain a professional tone at all times
- When you lack sufficient information, use web search to find current and accurate data
- If a question is ambiguous or unclear, ask the user for clarification before proceeding
- Cite sources when referencing specific facts or data from web research
- Be concise but comprehensive — do not omit important details
- If you are unsure about something, state your level of confidence honestly
...
4. If the question requires current information or verification, use web search tools
5. Synthesize information and present a clear, professional response

Formatting Guidelines:
- Use clear headings and bullet points for complex answers
- Keep responses focused and relevant
- Provide brief summaries for lengthy topics unless detail is requested""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8023",
        "frontend_url": f"http://localhost:8024",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8023")
    print(f"[FRONTEND] Frontend URL: http://localhost:8024")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8023/docs")
    print(f"[HEALTH] Health: http://localhost:8023/health")
